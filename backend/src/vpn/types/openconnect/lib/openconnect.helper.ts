...
export const connect = async (
  ...
  authgroup: string,
  name: string,
  vendor: 'anyconnect' | 'gp' | 'fortinet' | 'openconnect',
) => {
  try {
    await exec(
      `echo ${password} | sudo openconnect --protocol=${vendor} --background --servercert pin-sha256:q+pYWBNW7f5OY5O9kGPXXwTCT4kEJS8ix6OhSXZrzKs= --user=${username} --non-inter --authgroup=${authgroup} --interface=${name} --passwd-on-stdin ${remote}`,
      {
