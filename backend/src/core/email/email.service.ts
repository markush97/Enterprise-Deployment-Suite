...
export class EMailService {
  ...
  async sendEmail(subject: string, html: string, receiver?: string | string[]): Promise<void> {
    if (!receiver) {
      receiver = this.emailConfig.defaultReceiver;
      if (receiver === null) {
        throw new InternalMTIException(
          MTIErrorCodes.EMAIL_NO_RECEIVER,
          'No receiver or default receiver configured. Cannot send email!',
        );
      }
    }
