...
export class NetworkService implements OnModuleInit {
  ...
  constructor(
    ...
    private readonly networkRepository: EntityRepository<NetworkInterfaceEntity>,
  ...
  async updateDHCPConfig(
    ...
    dhcpConfig: Partial<ConfigureDHCPDto>,
    ...
    if (!networkInterface) {
      throw new BadRequestMTIException(
        MTIErrorCodes.UNKNOWN_INTERFACE,
        'Cannot configure unknown interface!',
      );
    }
  ...
  private async syncInterfaces(systemInterfaces: Record<string, NetworkInterface>): Promise<void> {
    ...
    const removedInterfaces = dbInterfaces.filter(iface => !systemInterfaces[iface.name]);
  ...
  private getSystemInterfaces(): Record<string, NetworkInterface> {
    ...
    for (const [name, addresses] of Object.entries(systemInterfaces)) {
      const filteredAddresses = addresses?.filter(
        addr =>
          (!this.networkConfig.ipv4Only || addr.family === 'IPv4') &&
          (this.networkConfig.includeInternalNetworks || !addr.internal),
      );
