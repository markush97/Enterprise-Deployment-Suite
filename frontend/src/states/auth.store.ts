  ...
  persist(
    (set, get) => ({
      ...
      loginWithEntraId: async (entraIdAccessToken: string) => {
        try {
          const response = await authApi.post(`${AUTH_ENTRAID_URL}/login`, undefined, {
            headers: { Authorization: `Bearer ${entraIdAccessToken}` },
          });
      ...
      onRehydrateStorage: () => async state => {
        ...
        if (state?.authToken) {
          try {
            ...
            await api.post('/auth/validate', undefined, {
              responseType: 'text',
