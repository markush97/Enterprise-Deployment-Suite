import { ArrowLeft, Building2, Check, Download, Edit, Files, Trash2, X } from 'lucide-react';
...
export function TaskDetail({
  ...
  onTaskDeleted,
  editMode,
}: TaskDetailProps & { editMode?: boolean }) {
  ...
  useEffect(() => {
    let isMounted = true;
    taskService
      .getTaskContentOverview(task.id)
      .then(overview => {
        if (isMounted) setHasContent(!!overview);
      })
      .catch(() => {
        if (isMounted) setHasContent(false);
      });
    return () => {
      isMounted = false;
    };
      ...
      <div className="bg-white dark:bg-gray-800 shadow rounded-lg">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
          ...
          <div className="flex space-x-2">
            <Tippy
              content="Cannot upload content for built-in tasks"
        ...
        <div className="px-6 py-4">
          <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
            <div>
              ...
              <dd className="mt-1 text-lg text-gray-900 dark:text-white">{task.name}</dd>
            </div>
            <div>
              <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">Description</dt>
              <dd className="mt-1 text-lg text-gray-900 dark:text-white">{task.description}</dd>
            </div>
            <div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg max-w-md w-full p-6 relative animate-fadeIn">
            ...
            <p className="mb-4 text-sm text-gray-600 dark:text-gray-300">
              Please select a <span className="font-semibold">.zip</span> file. Only zip-archives
              are allowed. The uploaded content will be attached to this task and can be used for
