                ...
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                {columns.map((col, i) => (
                                    <th key={i} className={col.className || 'px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider'}>
                                        {col.label}
                                    </th>
                                ))}
                                {actions.length > 0 && (
                        ...
                        <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                            {isLoading ? null : data.length === 0 ? (
                                <tr>
                                    <td colSpan={columns.length + (actions.length > 0 ? 1 : 0)} className="px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                                        {emptyState}
                                    </td>
                                </tr>
                            ) : (
                                    ...
                                    >
                                        {columns.map((col, i) => (
                                            <td key={i} className={col.className || 'px-6 py-4 whitespace-nowrap'}>
                                                {col.render ? col.render(item) : (col.accessor ? (item as any)[col.accessor] : null)}
                                            </td>
                                        ))}
                                        {actions.length > 0 && (
