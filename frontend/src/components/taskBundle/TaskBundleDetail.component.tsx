import { ArrowLeft, Building2, Download, Edit, Trash2 } from 'lucide-react';
...
export function TaskBundleDetail({
  taskBundle: initialTaskBundle,
  onBack,
  onTaskBundleDeleted,
  editMode,
  onTaskBundleUpdated,
}: TaskBundleDetailProps & { editMode?: boolean }) {
  const [taskBundle, setTaskBundle] = useState<TaskBundle & { customers?: { id: string }[] }>(
    initialTaskBundle,
  );
  ...
  useEffect(() => {
    setLoading(true);
    taskBundleService
      .getTaskBundle(initialTaskBundle.id)
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{taskBundle.name}</h1>
      </div>
...
function CustomerAssignmentCard({
  ...
  taskBundle: TaskBundle & { customers?: { id: string }[] };
}) {
  ...
  const initialIds =
    taskBundle.customerIds && taskBundle.customerIds.length > 0
      ...
      : taskBundle.customers
        ? taskBundle.customers.map(c => c.id)
        : [];
    ...
    return (
      <div className="bg-white dark:bg-gray-800 shadow rounded-lg px-6 py-4 mb-4">
        ...
        <div className="text-gray-600 dark:text-gray-300">
          Cannot assign customers since this bundle is global. Every customer can use global
          bundles.
        </div>
      </div>
    );
          ...
          {customers.map(customer => (
            <label key={customer.id} className="flex items-center gap-2 mb-1 cursor-pointer">
              <input
                ...
                disabled={saving}
              />
              <span className="font-mono text-sm text-gray-800 dark:text-gray-200 w-24 truncate">
                {customer.shortCode}
              </span>
