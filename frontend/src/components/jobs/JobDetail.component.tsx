...
export function JobDetail({ job, onBack, onJobUpdated, onJobDeleted, editMode }: JobDetailProps) {
  ...
  >([]);
  const [taskBundleOptions, setTaskBundleOptions] = useState<
    { id: string; name: string; description?: string; taskList?: Task[] }[]
  >([]);
  ...
  const getTaskBundleId = () => {
    if (job.taskBundle && typeof job.taskBundle === 'object' && 'id' in job.taskBundle)
      return (job.taskBundle as any).id;
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{job?.name ?? job.id}</h1>
      </div>
        ...
        <div className="px-6 py-4">
          <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
            ...
            <div>
              ...
              <dd className="mt-1 text-sm text-blue-600 dark:text-blue-400">
                {'id' in (job.taskBundle || {}) ? (
                  <a
                    href={`/taskbundles/${(job.taskBundle as any).id}`}
            ...
            <div>
              ...
              <dd className="mt-1 text-sm text-gray-900 dark:text-gray-100">
                {new Date(job.createdAt).toLocaleString()} by {job.startedBy?.name}
              </dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd className="mt-1 text-sm text-gray-900 dark:text-gray-100">
                {job.lastConnection ? new Date(job.lastConnection).toLocaleString() : 'N/A'}
              </dd>
            </div>
          </dl>
        </div>
      ...
      {!isJobStarted && (
        <div className="flex justify-center my-8">
          <Tippy
            content={
              !job.customer || !job.taskBundle
                ? 'Assign both a customer and a task bundle before starting the job.'
                : ''
            }
          ...
          <div className="bg-white dark:bg-gray-900 p-8 rounded shadow-lg w-full max-w-lg">
            ...
            <p className="mb-4 text-gray-700 dark:text-gray-200">
              ...
              <span className="font-semibold text-red-600 dark:text-red-400">
                After starting, changes to the customer or task bundle are no longer possible.
              </span>
            </p>
            <div className="mb-4 space-y-2">
              <div>
                <span className="font-semibold">Task Bundle:</span>{' '}
                {job.taskBundle?.name || (job.taskBundle as any)?.name || 'N/A'}
              </div>
              <div>
                <span className="font-semibold">Customer:</span>{' '}
                {job.customer?.shortCode ||
                  (job.customer as any)?.shortCode ||
                  job.customer?.name ||
              ...
              <div>
                ...
                {job.device?.name ||
                  ...
                  (job.device as any)?.name ||
                  (job.device as any)?.serialNumber ||
                  'N/A'}
              </div>
            </div>
            <div className="flex justify-end space-x-2">
      ...
      <AssignTaskBundleCard
        ...
        handleAssign={handleAssign}
        tooltip={
          isJobStarted ? 'Task bundle cannot be changed after the job has started.' : undefined
        }
      />
    </div>
  );
