                ...
                <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                    {sortedTokens.map(token => (
                        <li key={token.id} className="py-2 flex items-center justify-between">
                            <div>
                                <div className="text-sm text-gray-900 dark:text-white">Device: {token.device || 'Unknown'}</div>
                                <div className="text-xs text-gray-500 dark:text-gray-400">Created: {token.createdAt}</div>
                                <div className="text-xs text-gray-500 dark:text-gray-400">Last Used: {token.lastUsedAt || 'Never'}</div>
                                {token.isCurrent && <span className="text-xs text-green-600 ml-2">Current session</span>}
                            </div>
                            {!token.isCurrent && (
                                <button
